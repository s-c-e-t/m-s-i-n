...
def render_card_content(c, clue, x, y):
    ...
    if puzzle_img_name:
        ...
        # Fallback to text
        draw_wrapped_text(c, clue.get("text", ""), x + 0.5*cm, content_top, content_width)
...
def render_back_content(c, clue, x, y):
    ...
    if text:
        ...
        # Approximate height for centering (cap height)
        # ReportLab doesn't give easy font metrics for cap height, estimating ~0.7 * size
...
def create_pdf():
    ...
    for i in range(0, len(clues), cards_per_page):
        ...
        # Calculate grid dimensions for this batch (centering logic)
        # Note: We center based on the full grid structure even if the batch is partial,
        # OR we center the partial batch?
        # "Centered on the page" usually implies the grid is fixed to the page center.
        # But if the last page has only 1 card, should it be in the corner or center?
        # User said "each clue/card centered". Let's center the actual content.
        ...
        for idx, clue in enumerate(batch):
            ...
            # If the last row is not full, should we center the row?
            # Standard grid layout aligns to the left of the grid block.
            # But the grid block itself is centered.
            # If full grid width is used (actual_cols = cols), it's fine.
            # If last row has fewer items, they will be left-aligned within the centered grid block.
        ...
        if DOUBLE_SIDED:
            for idx, clue in enumerate(batch):
                ...
                # If we have `actual_cols` columns in the grid
                # The visual block is centered.
                # Leftmost column (0) on Front becomes Rightmost (actual_cols - 1) on Back.
